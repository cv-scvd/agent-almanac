...
### Added
- **`workflows/verify-handoff.mjs`** — adversarially verifies a `CONTINUE_HERE.md` draft against a facts file (each fact naming the command that produced it) and its sources, three lenses per draft in parallel: traceability, completeness against the previous edition or plan, actionability. Promoted from the 2026-09-02 session scratchpad, where two ad-hoc runs of this shape caught a heartbeat claim extrapolated over five unread days, a "six phases" count for a seven-phase plan, and a Context section claimed verbatim whose Links block had been rewritten. `write-continue-here` gains the matching Validation items and three pitfalls.
- **`tools/review-bundle.sh`** — builds the self-contained bundle an adversarial reviewer needs when it must not read the working tree: diff against a base ref, full post-change copies of every added or modified source file, the PR body, and a README that says to read nothing else; `--summarise` reports mirror-heavy paths as a stat plus a sample. `--verify` runs it in a throwaway repo and asserts nothing changed is omitted and nothing summarised is expanded.
- **`tools/merge-dependabot.sh`** — merges open Dependabot PRs oldest-first, re-polling mergeability before each merge because the shared lockfile flips the rest to UNKNOWN and then CONFLICTING; reads the verdict from the API, never from gh's stderr; asks Dependabot to rebase on a conflict. The decision table is a function and `--verify` pins it.
